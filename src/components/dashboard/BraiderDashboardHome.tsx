...
import { JOBS, APPLICATIONS } from "@/lib/sampleData";
...
export function BraiderDashboardHome({ firstName }: { firstName: string }) {
    ...
    <>
      ...
      <div style={{ padding: 32, display: "flex", flexDirection: "column", gap: 26 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 18 }}>
          <StatCard icon={<BriefcaseIcon />} label="Jobs near you"  value="24"     tone="brand" />
          <StatCard icon={<SendIcon />}      label="Applications"   value="6"      tone="gold" />
          <StatCard icon={<MessageIcon />}   label="Salon replies"  value="3" delta="2 new" tone="sage" />
          <StatCard icon={<DollarIcon />}    label="Avg offer"      value="$32/hr" tone="teal" />
        </div>
              ...
              {nearby.map((j, i) => (
                <div key={j.id} style={{ display: "flex", alignItems: "center", gap: 14, padding: 14, border: "1px solid var(--border-subtle)", borderRadius: "var(--radius-md)" }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 13, color: "var(--text-muted)" }}>{j.salon} · {j.city} · {j.distance}</div>
                  </div>
